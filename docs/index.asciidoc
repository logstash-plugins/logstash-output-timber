...
This output sends events to the Timber.io logging service.

It is based off of the excellent (generic) HTTP output and modified for communication with
the Timber.io service. It's a highly efficient output using buffering, batching, async delivery,
and retries for reliable delivery.
